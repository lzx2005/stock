...
| 前端（第六期） | Node ≥ 20 + npm；`frontend/`（Vite5 + React18 + antd5 + ECharts5）：`npm run dev` 开发（proxy→8666）/ `npm run build` 出 `dist` |
| API Key | `TICKFLOW_API_KEY` 在 `~/.zshrc`；**非交互 shell 不加载 `.zshrc`，跑真实数据前先 `source ~/.zshrc`** |
| 测试 | `.venv/bin/python -m pytest tests/`（`--timeout=60`），当前 **200 全绿** |
| 数据 | `data/`（已 gitignore，勿提交）：`klines/`（parquet 分区）、`meta.db`、`factors/`、`strategies/`（策略库 JSON） |
| 权限实测 | 分钟 K **已可用**（深度限最近一年）；WS 实时推送**无权限**（代码已实现，待开通） |
| 前端视觉 | 「本地行情终端」浅色风：红涨绿跌语义色、深墨蓝报价头、数字等宽对齐；纯系统字体（离线约束，不引 web font） |

## 4. 迭代历史（7 期已完成 ✅；第六期 Web 界面待人工冒烟；200 测试全绿）
...
| 八期 | monitor-phase8 | 日内盯盘系统 | spec `2026-09-05-intraday-monitor-design.md` + 计划已写待实施：独立守护进程+monitor.db（tasks/signals 两表，脚本代码存库内）+边沿亮灭灯提醒+Bark 推送写死（key 在 `data/monitor_config.json`，已实测通过 `scripts/test_bark.py`）+webui 第三 tab+`scripts/monitor_cli.py`+盯盘专家 skill（自然语言→check(ctx) 脚本→验证/注册/诊断） |
...
└── webui/                # Web 查询界面（第六期）
    ├── app.py            # create_app(data_dir) 工厂：三只读存储（KlineStore/MetaStore/FactorStore）；生产挂 frontend/dist
    └── api.py            # /api 只读端点：periods/coverage/klines/factors/factor-dirs/factor-values（scripts/serve.py 启动 8666）
frontend/                 # 前端（第六期）：Vite+React+antd+ECharts（K线/因子浏览两 tab，行情终端风：红涨绿跌+深色报价头+左右分栏），npm run dev/build
```
...
└── superpowers/
    ...
    └── plans/               # 实施计划（checkbox 版，进度索引）：phase1~phase7
```
...
```bash
source ~/.zshrc
.venv/bin/python -m pytest tests/                    # 全量测试（200）
...
.venv/bin/python scripts/serve.py                    # 启动 Web 查询后端（127.0.0.1:8666，真实数据只读）
cd frontend && npm run build                         # 前端生产构建（产出 frontend/dist，serve.py 同端口服务）
cd frontend && npm run dev                           # 前端开发模式（5173，/api 经 proxy → 8666）
